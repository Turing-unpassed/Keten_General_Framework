...
{
    // 保险起见，也算是把它的报文尽其用了
    if((motor_id & 0x400000) != GO1_Rec_From_Module)// 判断是不是发回来的报文
    {
        return -1;
    }    
    uint8_t temp_motor_id = (motor_id & 0xF00);
...
uint8_t GO_M8010::GO_Motor_Standard_Ctrl(float ref_kpos,float ref_kspd,float ref_torque,float ref_speed,float ref_pos)
{
    ...
    this->Set_Ref_CtrlParam(ref_torque,ref_speed,ref_pos);
    if(this->last_ref_kpos != ref_kpos || this->last_ref_kspd != ref_kspd)
    {
        // 想要修改Kpos 和 Kspd
        this->Set_Ref_KParam(ref_kpos,ref_kspd);
        this->Set_Send_Extid(CAN_To_RS485_Module_ID_3|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_2|GO1_Motor_ID_0|GO1_Default_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte);
        tx_id = CAN_To_RS485_Module_ID_3|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_2|GO1_Motor_ID_0|GO1_Default_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte;
        bbb+=0.001;
    }
    else
    {
       // 说明Kpos 和 Kspd没有变化，不需要再使用控制模式11发送一次，直接使用控制模式10发送
        this->Set_Send_Extid(CAN_To_RS485_Module_ID_3|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_1|GO1_Motor_ID_0|GO1_FOC_Closer_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte);
        tx_id = CAN_To_RS485_Module_ID_3|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_1|GO1_Motor_ID_0|GO1_FOC_Closer_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte;
        fff+=0.001;
    }
    this->CANMsg_Process();
    
    this->last_ref_kpos = ref_kpos;
    this->last_ref_kspd = ref_kspd;
    if(this->can_tx_for_motor.can_handle == &hcan1)
        xQueueSend(CAN1_TxPort,&this->can_tx_for_motor,portMAX_DELAY);
    if(this->can_tx_for_motor.can_handle == &hcan2)
        xQueueSend(CAN2_TxPort,&this->can_tx_for_motor,portMAX_DELAY);
...
uint8_t GO_M8010::GO_Motor_SaveResource_Ctrl(float ref_kpos,float ref_kspd,float ref_torque,float ref_speed,float ref_pos)
{
    ...
    if(this->last_ref_kpos != ref_kpos || this->last_ref_kspd != ref_kspd)
    {
        // 想要修改Kpos 和 Kspd
        this->Set_Ref_KParam(ref_kpos,ref_kspd);
        this->Set_Send_Extid(this->module_id|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_2|this->motor_id|GO1_Default_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte);
    }
    else
    {
        // 说明Kpos 和 Kspd没有变化，不需要再使用控制模式11发送一次，直接使用控制模式13发送
        this->Set_Send_Extid(this->module_id|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_4|this->motor_id|GO1_FOC_Closer_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte);
    }
    this->CANMsg_Process();
...
uint8_t GO_M8010::GO_Motor_ReadBack_Ctrl()
{
    this->Set_Send_Extid(this->module_id|GO1_Send_To_Module|GO1_Send_Data_Mode|GO1_Ctrl_Mode_3|this->motor_id|GO1_FOC_Closer_Mode|GO1_Send_Null_1Bits|GO1_Send_Null_1Byte);
    this->CANMsg_Process();
    if(this->can_tx_for_motor.can_handle == &hcan1)
        xQueueSend(CAN1_TxPort,&this->can_tx_for_motor,portMAX_DELAY);
    if(this->can_tx_for_motor.can_handle == &hcan2)
...
void GO_M8010::update_Go1(uint8_t can_rx_data[],uint32_t data_id)
{
    ...
    if((data_id & 0x3000000) == GO1_Rec_Data_Mode1)// 接收数据模式1
    {
        ...
        {
            // 电机发回来报错报文！
            this->errorType_Get((data_id & 0xFF0000) >> 16);// 进行错误代号解析
        }
        else
        {
            // 电机正常发回来报文！这时记得存储温度
